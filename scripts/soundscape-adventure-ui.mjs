...
class SoundscapeAdventureUI {
    display_currentmood_sounds=true;
    constructor() {
    ...
    // TODO Create a status that save if the Current Mood is collapsed or not
    // TODO Same for the soundscapes, save a config to check if it is collapsed or not
    async _createMoodControls(html) {
        // it might requires a config to save the collapse statuses for each soundscape
        // it might requires a config to save the statuses for the current mood
        ...
        if (this.moodControlsElement.length == 0) {
            ...
            if (globalVolumeElement) {
                ...
                utils.log(utils.getCallerInfo(),"Elements with class global-volume or current-playing not found", constants.LOGLEVEL.ERROR);
            }
        }
    }
